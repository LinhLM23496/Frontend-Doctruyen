...
import { ListProps, RefListType } from 'components/List/types'
import { List, Text } from 'components'
import { space } from 'themes'
import { UseBookType } from 'stores/books/types'

type QureyListType = Omit<ListProps, 'data'> & {
  queryHook: Function
...
const QueryList = forwardRef((props: QureyListType, ref?: RefListType) => {
  const {
    Element,
    queryHook,
    renderItem,
  ...
  const renderData: ListRenderItem<any> = ({ item, index }: RenderItemType) => {
    if (item === 'loading') {
      return (
